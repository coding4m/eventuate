...
//    .settings(protocSettings: _*)
  .settings(integrationTestSettings: _*)
  .settings(libraryDependencies ++= Seq(AkkaRemote, Protobuf))
...
lazy val logElastic = (project in file("eventuate-log-elastic"))
  .dependsOn(core % "compile->compile;it->it;multi-jvm->multi-jvm")
  .settings(name := "eventuate-log-elastic")
  .settings(commonSettings: _*)
  .settings(integrationTestSettings: _*)
...
lazy val logRocksdb = (project in file("eventuate-log-rocksdb"))
  .dependsOn(core % "compile->compile;it->it;multi-jvm->multi-jvm")
  .settings(name := "eventuate-log-rocksdb")
  .settings(commonSettings: _*)
  .settings(integrationTestSettings: _*)
  .settings(libraryDependencies ++= Seq(AkkaRemote, Rocksdb))
  .settings(libraryDependencies ++= Seq(AkkaTestkit % "test,it", AkkaTestkitMultiNode % "test", Scalatest % "test,it"))
  .settings(jvmOptions in MultiJvm += "-Dmultinode.server-port=4713")
  .configs(IntegrationTest, MultiJvm)
  .enablePlugins(HeaderPlugin, AutomateHeaderPlugin)

lazy val logAdapter = (project in file("eventuate-log-adapter"))
//  .dependsOn(logElastic % "compile->compile;it->it;multi-jvm->multi-jvm")
  ...
  .dependsOn(logRocksdb % "compile->compile;it->it;multi-jvm->multi-jvm")
  .settings(name := "eventuate-log-adapter")
  .settings(commonSettings: _*)
  .settings(integrationTestSettings: _*)
  .settings(libraryDependencies ++= Seq(AkkaTestkit % "test,it", AkkaTestkitMultiNode % "test", Scalatest % "test,it"))
  .settings(jvmOptions in MultiJvm += "-Dmultinode.server-port=4713")
