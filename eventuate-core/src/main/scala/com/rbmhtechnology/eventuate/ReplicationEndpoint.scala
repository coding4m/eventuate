...
object ReplicationEndpoint {
  ...
  def apply(logFactory: String => Props)(implicit system: ActorSystem): ReplicationEndpoint = {
    ...
    val maxConnections = config.getInt("eventuate.endpoint.max-connections")
    apply(logFactory, connections, connectionRoles, maxConnections)
  }
   ...
   * @param connections Replication connections to other replication endpoints.
   */
  def apply(logFactory: String => Props, connections: Set[ReplicationConnection], connectionRoles: Set[String], maxConnections: Int)(implicit system: ActorSystem): ReplicationEndpoint = {
    val config = system.settings.config
    ...
    new ReplicationEndpoint(endpointId, endpointLogs, logFactory, connections, connectionRoles, maxConnections, applicationName = applicationName, applicationVersion = applicationVersion)(system)
  }
