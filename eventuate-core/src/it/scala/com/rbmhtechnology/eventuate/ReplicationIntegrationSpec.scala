...
trait ReplicationIntegrationSpec extends WordSpec with Matchers with MultiLocationSpec {
  ...
  "Event log replication" must {
    ...
    "replicate events based on local and remote filter criteria" in {
      ...
      val endpointA = locationA.endpoint(Set("L1"), Set(replicationConnection(locationB.port)), sourceFilters(Map("L1" -> new PayloadInequalityFilter("a2"))))
      val endpointB = locationB.endpoint(Set("L1"), Set(replicationConnection(locationA.port)), sourceFilters(Map("L1" -> new PayloadInequalityFilter("b2"))))
